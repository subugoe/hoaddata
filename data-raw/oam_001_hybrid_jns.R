...
cr_title_list <-
  ...
  dplyr::mutate(years = stringr::str_extract_all(
...
cr_issn_list <- cr_title_list |>
  ...
  dplyr::mutate(
    issn_norm =
      stringi::stri_sub_replace(issn_norm, 5, 4, value = "-")
  )
...
oam_cr <- oam_new |>
  ...
  dplyr::mutate(years = purrr::map(
    years, function(x) grep("^20[0-2]", x, value = TRUE))
    ) |>
  ...
  dplyr::filter(!cr_journal_id %in% c(
    180565, 14676, 32798, 170806,
