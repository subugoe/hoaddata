...
FROM (
  SELECT
    ...
    has_ref
  FROM
    `hoad-dash.jct.cr_raw` AS raw,
    UNNEST(link) AS link,
    UNNEST(author) AS author 
    INNER JOIN `hoad-dash.jct.cc_md` as cc_md ON raw.doi = cc_md.doi
    INNER JOIN `hoad-dash.jct.cr_openalex_inst_full` as oalex ON raw.doi = oalex.doi
    WHERE
   ( vor = 1
    AND NOT cc IS NULL) AND country_code = "DE") )

SELECT cr_year, esac_publisher,
...
FROM md_coverage
GROUP BY cr_year, esac_publisher